#!/usr/bin/env python3
"""
Facebook Messenger Automation Bot - FINAL STABLE VERSION
"""
...
# UI Styles (Same as before, looks good)
custom_css = """
<style>
...
# Session State & Automation State (Looks Good)
...
# setup_browser function (Same, looks good for Streamlit Cloud)
...
# find_message_input function (Same, keep for logging HTML)
def find_message_input(driver, process_id, automation_state=None):
    ...
    message_input_selectors = [
        ...
        # Accessibility Label-based
        'div[data-testid="mwchat-composer-input"]',
        
        # Generic Text Area
        'div[aria-describedby^="placeholder"]',
        'div[role="presentation"] div[contenteditable="true"]',
        'div[role="log"] + div div[contenteditable="true"]',
                    ...
                    if is_editable:
                        ...
                        if element.is_displayed() and element.is_enabled():
                            ...
                            if any(keyword in element_text for keyword in keywords):
                                ...
                                log_message(f'{process_id}: Using advanced selector element', automation_state)
                                return element
                except Exception as e:
...
# send_messages function (Database calls removed)
def send_messages(config, automation_state, user_id, process_id='AUTO-1'):
    ...
    try:
        ...
        if not message_input:
            ...
            # set_automation_running(user_id, False) # ✅ REMOVED: STABILITY FIX
            return 0
        ...
        while automation_state.running and messages_sent < 100:
            ...
            try:
                ...
                if not sent:
                    ...
                    log_message(f'{process_id}: Sent via Enter key', automation_state)
                    sent = True
                
                # Fallback to JavaScript Enter event
                if not sent:
                    driver.execute_script("""
                        const element = arguments[0];
                    ...
                    """, message_input)
                    log_message(f'{process_id}: Sent via JavaScript event', automation_state)
...
# ✅ THREADING APPROACH WITH RERUN AT END OF THREAD
def run_automation_with_notification(user_config, username, automation_state, user_id):
    send_messages(user_config, automation_state, user_id)
    # 💥 THREAD KHATAM HONE KE BAAD DB UPDATE
    set_automation_running(user_id, False)
    # 💥 THREAD KHATAM HONE KE BAAD RERUN (Safe way)
    st.rerun() 
            ...
            with tab2:
                ...
                with col1:
                    if st.button("Start Automation", disabled=st.session_state.automation_state.running, use_container_width=True):
                        ...
                        if current_config and current_config['chat_id']:
                            ...
                            # st.rerun() is in start_automation
                        else:
                ...
                with col2:
                    if st.button("Stop Automation", disabled=not st.session_state.automation_state.running, use_container_width=True):
                        stop_automation(st.session_state.user_id)
                        # st.rerun() is in stop_automation
                ...
                # ✅ YEH BLOCK HATA DIYA HAI - FLASHING/RECURSION ERROR KA KAARAN
                # if st.session_state.automation_state.running:
                #     time.sleep(1)
                #     st.rerun() 
